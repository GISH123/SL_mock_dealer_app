...
class WheelCanvas:
    ...
    def _load_images(self):
        """Load PNG images from ./card_shown_ui if available."""
        img_dir = os.path.join(os.path.dirname(__file__), 'card_shown_ui')
...
class MockDealerBall3Protocol(asyncio.Protocol):
    ...
    # Dealer -> detector commands
    def send_start_predict(self):
        # Use app-generated round_id (14 bytes max on wire)
        self.game_timestamp = self.app.current_round_id or self._new_gmcode()
        gmcode = self.game_timestamp.encode('utf-8')[:14].ljust(14, b'\x00')
        gmstate = 2
        body = struct.pack('!14sh', gmcode, gmstate)
        packet = struct.pack('!3I', CMD_START_PREDICT, 12 + len(body), 0) + body
        self.transport.write(packet)
        self.app.log(f'[PyDealer] Sent CMD_START_PREDICT gmcode={self.game_timestamp}')
    ...
    def send_stop_predict(self):
        ...
        self.transport.write(packet)
        self.app.log('[PyDealer] Sent CMD_STOP_PREDICT.')
...
class MockDealerBall3App:
    def __init__(self, host='127.0.0.1', port=2331):
        ...
        try:
            src = getattr(self, '_stable_times_source', {}) or {}
        except Exception:
            pass
